...
def save_data(
    docs: List[Document],
    output_dir: str = "user_files", 
    output_file: str = "user_document.txt"
):
    """
    Save the documents to a single text file in the system.

    Parameters:
    docs (List[Document]): A list of documents to be saved.
    output_dir (str): The directory where the text file will be saved.
    output_file (str): The name of the output text file.

    Returns:
    int: The number of documents saved.
    """
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        for doc in docs:
            # Write the page_content to the file
            f.write(doc.page_content)
            # Add a newline to separate each document's content
            f.write('\n')
...
def upload_data(docs: List[Document]):
